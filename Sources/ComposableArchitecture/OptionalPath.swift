...
public struct OptionalPath<Root, Value> {
    ...
    public init<OldValue>(_ keyPath: WritableKeyPath<Root, OldValue>, _ path: CasePath<OldValue, Value>) {
        let optionalPath = OptionalPath<Root, OldValue>(
            extract: { $0[keyPath: keyPath] },
            set: { $0[keyPath: keyPath] = $1 }
        )
        self.init(
...
public extension Reducer {
    func pullback<GlobalState, GlobalAction, GlobalEnvironment, StatePath, ActionPath>(
    ...
    where
        ...
        return .init { globalState, globalAction, globalEnvironment in
            ...
            let effect =
                self
                .run(&localState, localAction, toLocalEnvironment(globalEnvironment))
